...
If you clear your browser cache and browser data while the plugin is installed, the mobile browser may fail to load all panels and meta-properties required to run plugins on the next visit, resulting in a blank screen. This appears to be a Thymer issue with restoring panel layout state after a cache clear.
